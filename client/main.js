...
function groupIncidentMessagesByUser(messages) {
  ...
  messages.forEach((msg) => {
    ...
    group.handling_status = msg.handling_status || group.handling_status || "NOT_HANDLED";
...
function renderAdminIncidentUsers() {
  ...
  list.innerHTML = groups.map((group) => {
    const statusInfo = getIncidentHandlingInfo(group.handling_status);
    return `
      <button type="button" class="incident-thread-btn ${String(group.user_id) === String(activeIncidentUserId) ? "active" : ""}" data-user-id="${escapeHtml(group.user_id)}">
        ...
        <em class="handling-badge ${statusInfo.className}">${statusInfo.text}</em>
      </button>
    `;
  }).join("");
...
function renderAdminIncidentThread() {
  ...
  if (title) {
    const statusInfo = getIncidentHandlingInfo(group.handling_status);
    title.innerHTML = `
      <span>محادثة: ${escapeHtml(group.user_name)} - ${escapeHtml(group.user_email)}</span>
      <span class="handling-badge ${statusInfo.className}">${statusInfo.text}</span>
      <select id="adminIncidentHandlingStatus" class="handling-status-select" aria-label="حالة التعامل">
        <option value="NOT_HANDLED" ${group.handling_status === "NOT_HANDLED" ? "selected" : ""}>لم يتم التعامل</option>
        <option value="IN_PROGRESS" ${group.handling_status === "IN_PROGRESS" ? "selected" : ""}>جارٍ التواصل</option>
        <option value="DONE" ${group.handling_status === "DONE" ? "selected" : ""}>تم التعامل</option>
      </select>
    `;
  }
...
function initAdminIncidentChat() {
  ...
  $("#adminPanelIncident")?.addEventListener("change", async (e) => {
    if (e.target.id !== "adminIncidentHandlingStatus" || !activeIncidentUserId) return;
    const handling_status = e.target.value;
    try {
      await apiFetch(`/admin/incident-chat/${activeIncidentUserId}/status`, {
        method: "PUT",
        body: { handling_status }
      });
      adminIncidentMessagesCache = adminIncidentMessagesCache.map((msg) => (
        String(msg.user_id) === String(activeIncidentUserId) ? { ...msg, handling_status } : msg
      ));
      renderAdminIncidentUsers();
      showToast("تم تحديث الحالة", "تم حفظ حالة التعامل مع رسالة الحادث");
    } catch (err) {
      showToast("فشل تحديث الحالة", err.message || "تعذر حفظ الحالة", "error");
    }
  });
  ...
  loadAdminIncidentMessages().catch(() => renderAdminIncidentUsers());
}

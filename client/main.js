    ...
    return `
      <article class="admin-request-card user-request-card">
        <div class="admin-request-head">
          <h3>${escapeHtml(item.title_ar || item.title_en || item.car_model || "طلب تأمين")}</h3>
...
function initRiskPage() {
  ...
  form?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      showToast("فشل التحليل", err.message || "تعذر تحليل البيانات", "error");
    } finally {
      btn.disabled = false;
      btn.textContent = "تحليل المخاطر بالذكاء الاصطناعي";
    }
...
function renderAdminRiskAnalyses(items) {
  ...
  if (!wrap) return;
  if (!items.length) {
    wrap.innerHTML = '<div class="empty-state">لا توجد تحليلات AI حالياً.</div>';
    return;
  }
  wrap.innerHTML = items.map((item) => `
    <article class="admin-request-card risk-admin-card">
      ...
      <p><strong>المركبة:</strong> ${escapeHtml(item.vehicle_type || "-")} / ${escapeHtml(item.car_year || "-")}</p>
      <p><strong>الاستخدام:</strong> ${escapeHtml(item.usage_type || "-")}</p>
...
function attachAdminRequestActions() {
  ...
  wrap.addEventListener("click", async (e) => {
    ...
    try {
      ...
      renderAdminRequests(Array.isArray(items) ? items : []);
    } catch (err) {
...
function initAdminPage() {
  ...
  apiFetch('/admin/ai/risk-analyses')
    .then((items) => renderAdminRiskAnalyses(Array.isArray(items) ? items : []))

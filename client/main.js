...
const AUTH_REQUIRED_PAGES = new Set(["request", "ai-risk", "license", "contact", "accidents", "accident-procedures", "incident-chat"]);
  ...
  <header class="site-header">
    <div class="container navbar">
      <a class="logo" href="index.html">
        <img src="assets/cov.png" alt="Covera" class="logo-img" />
        <span>Covera</span>
      </a>
      ...
      <ul class="nav-links" id="navLinks">
        ...
        <li><a href="laws.html" class="${activePage === "laws" ? "active" : ""}">القوانين</a></li>
        ...
        <li class="auth-required"><a href="accidents.html" class="${activePage === "accidents" ? "active" : ""}">الحوادث</a></li>
        <li class="auth-required"><a href="incident-chat.html" class="${activePage === "incident-chat" ? "active" : ""}">تواصل أثناء الحادث</a></li>
    ...
    <footer class="footer">
      <div class="container footer-inner">
        ...
        <div class="footer-links">
          ...
          <a class="auth-required" href="accidents.html">الحوادث</a>
          <a class="auth-required" href="incident-chat.html">تواصل أثناء الحادث</a>
...
const accidentBrandOptions = {
  "سيارة خاصة": ["Toyota", "Hyundai", "Kia", "Mercedes-Benz", "BMW", "Volkswagen", "Honda", "Nissan", "Peugeot", "Ford", "غير ذلك"],
  "دراجة نارية": ["BMW", "Kawasaki Ninja", "Yamaha", "Honda", "Suzuki", "KTM", "Ducati", "Harley-Davidson", "غير ذلك"],
  "شاحنة": ["Mercedes-Benz", "Volvo", "MAN", "Scania", "Isuzu", "Hino", "Mitsubishi Fuso", "DAF", "غير ذلك"]
};

const accidentBrandFactors = {
  "Mercedes-Benz": 1.45,
  "BMW": 1.55,
  "Volvo": 1.45,
  "MAN": 1.4,
...
function updateAccidentBrands() {
  const category = $("#accidentVehicleCategory")?.value || "";
  const brandSelect = $("#accidentVehicleBrand");
  if (!brandSelect) return;
  const brands = accidentBrandOptions[category] || [];
  brandSelect.innerHTML = '<option value="">اختر شركة / نوع المركبة</option>' + brands.map((brand) => `<option value="${escapeHtml(brand)}">${escapeHtml(brand)}</option>`).join("");
  brandSelect.disabled = !category;
}

function calculateAccidentEstimate(payload) {
  const categoryBase = {
    "سيارة خاصة": 750,
    "دراجة نارية": 420,
    "شاحنة": 1450
  ...
  const selectedDamages = Array.isArray(payload.damageLocations) ? payload.damageLocations : [payload.damageLocation].filter(Boolean);
  const damagesTotal = selectedDamages.reduce((sum, location) => sum + (damageBase[location] || 600), 0);
  const base = (categoryBase[payload.vehicleCategory] || 700) + damagesTotal;
  const total = Math.round(base * (accidentFactor[payload.accidentType] || 1) * (accidentBrandFactors[payload.vehicleBrand] || 1.1));
  const insuranceOpeningFee = 700;
  const deductibleEstimate = 900;
  const claimTotal = insuranceOpeningFee + deductibleEstimate;

  let recommendation = "التصليح الشخصي يبدو أوفر من فتح مطالبة تأمين.";
  let level = "منخفضة";

  if (total >= 2500 && total < 4500) {
    recommendation = "قارن بين تكلفة التصليح ورسوم فتح مطالبة التأمين قبل اتخاذ القرار.";
    level = "متوسطة";
  }

  if (total >= 4500 || total > claimTotal + 1200) {
    recommendation = "يفضل فتح مطالبة تأمين لأن تكلفة الإصلاح المتوقعة مرتفعة.";
    level = "مرتفعة";
  }

  return { total, claimTotal, recommendation, level };
}
...
function initAccidentsPage() {
  ...
  updateAccidentBrands();
  $("#accidentVehicleCategory")?.addEventListener("change", updateAccidentBrands);

  const form = $("#accidentEstimatorForm");
  const resultBox = $("#accidentEstimateResult");

  form?.addEventListener("submit", (e) => {
    e.preventDefault();

    const selectedDamageLocations = Array.from(document.querySelectorAll('input[name="damageLocation"]:checked')).map((checkbox) => checkbox.value);

    const payload = {
      vehicleCategory: $("#accidentVehicleCategory")?.value,
      vehicleBrand: $("#accidentVehicleBrand")?.value,
      accidentType: $("#accidentType")?.value,
      damageLocations: selectedDamageLocations
    };

    if (!payload.vehicleCategory || !payload.vehicleBrand || !payload.accidentType || payload.damageLocations.length === 0) {
      showToast("بيانات ناقصة", "يرجى اختيار جميع الحقول قبل التحليل", "error");
      return;
    }

    const estimate = calculateAccidentEstimate(payload);

    if (resultBox) {
      resultBox.classList.remove("hidden");
      resultBox.innerHTML = `
        <div class="result-header">
          <h3>نتيجة تحليل تكلفة الحادث</h3>
          <span class="request-status ${estimate.level === "مرتفعة" ? "rejected" : estimate.level === "متوسطة" ? "pending" : "approved"}">خطورة التكلفة: ${escapeHtml(estimate.level)}</span>
        </div>
        <div class="request-details-grid">
          <p><strong>فئة المركبة:</strong> ${escapeHtml(payload.vehicleCategory)}</p>
          <p><strong>الشركة / النوع:</strong> ${escapeHtml(payload.vehicleBrand)}</p>
          <p><strong>نوع الحادث:</strong> ${escapeHtml(payload.accidentType)}</p>
          <p><strong>مواقع الضرر:</strong> ${escapeHtml(payload.damageLocations.join("، "))}</p>
          <p><strong>تكلفة الإصلاح المتوقعة:</strong> ${estimate.total.toLocaleString("ar")} شيكل</p>
          <p><strong>تكلفة فتح مطالبة تقريبية:</strong> ${estimate.claimTotal.toLocaleString("ar")} شيكل</p>
          <p class="request-details-full"><strong>توصية النظام:</strong> ${escapeHtml(estimate.recommendation)}</p>
        </div>
        <p class="muted-note">هذا تقدير أولي للمساعدة في اتخاذ القرار، وقد تختلف التكلفة الفعلية حسب الورشة ونوع القطع وحجم الضرر.</p>
      `;
    }

    showToast("تم التحليل", "تم تقدير تكلفة الإصلاح وعرض التوصية");
  });
}

let adminRequestsCache = [];
let adminRiskAnalysesCache = [];
let adminIncidentMessagesCache = [];
let activeIncidentUserId = null;
...
function renderIncidentChatMessages(messages, hostSelector = "#incidentChatMessages") {
  const wrap = $(hostSelector);
  if (!wrap) return;
  if (!messages.length) {
    wrap.innerHTML = '<div class="empty-state">لا توجد رسائل بعد. اكتب رسالتك وأرفق صورة إذا احتجت.</div>';
    return;
  }
  wrap.innerHTML = messages.map((msg) => {
    const mine = String(msg.sender_role || "USER").toUpperCase() === "USER";
    const roleText = mine ? "أنت" : "الأدمن";
    return `
      <div class="incident-message ${mine ? "from-user" : "from-admin"}">
        <div class="incident-message-head">
          <strong>${roleText}</strong>
          <span>${escapeHtml(formatChatTime(msg.created_at))}</span>
        </div>
        ${msg.message ? `<p>${escapeHtml(msg.message)}</p>` : ""}
        ${msg.image_data ? `<a href="${msg.image_data}" target="_blank" rel="noopener"><img src="${msg.image_data}" alt="صورة الحادث" class="incident-chat-image" /></a>` : ""}
      </div>
    `;
  }).join("");
...
async function loadMyIncidentChat() {
  const messages = await apiFetch("/incident-chat/my");
  renderIncidentChatMessages(Array.isArray(messages) ? messages : []);
}

function initIncidentChatPage() {
  const session = requireLogin();
  if (!session) return;

  const nameField = $("#incidentUserName");
  const emailField = $("#incidentUserEmail");
  if (nameField) nameField.textContent = session.name || "المستخدم";
  if (emailField) emailField.textContent = session.email || "-";

  loadMyIncidentChat().catch(() => renderIncidentChatMessages([]));

  $("#incidentChatForm")?.addEventListener("submit", async (e) => {
    e.preventDefault();
    const messageEl = $("#incidentMessage");
    const imageEl = $("#incidentImage");
    const btn = $("#incidentSendBtn");
    const message = messageEl?.value.trim() || "";
    const file = imageEl?.files?.[0] || null;

    if (!message && !file) {
      showToast("رسالة فارغة", "اكتب رسالة أو أرفق صورة للحادث", "error");
      return;
    }

    if (file && !file.type.startsWith("image/")) {
      showToast("ملف غير مدعوم", "يمكن إرسال الصور فقط", "error");
      return;
    }

    if (file && file.size > 4 * 1024 * 1024) {
      showToast("الصورة كبيرة", "اختر صورة أقل من 4MB", "error");
      return;
    }
    ...
    try {
      ...
      await apiFetch("/incident-chat/messages", {
        method: "POST",
        body: {
          message,
      ...
      await loadMyIncidentChat();
      showToast("تم الإرسال", "وصلت رسالتك إلى الأدمن");
    } catch (err) {
      showToast("فشل الإرسال", err.message || "تعذر إرسال الرسالة", "error");
    } finally {
      if (btn) {
        btn.disabled = false;
  ...
  $("#incidentRefreshBtn")?.addEventListener("click", () => {
    loadMyIncidentChat().catch(() => showToast("تعذر التحديث", "حاول مرة أخرى", "error"));
  });
}

function incidentHandlingStatusText(status = "not_handled") {
  const map = {
    handled: "تم التعامل",
    communicating: "جارٍ التواصل",
    not_handled: "لم يتم التعامل"
  };
  return map[status] || map.not_handled;
}

function incidentHandlingStatusClass(status = "not_handled") {
  const map = {
    handled: "handled",
...
function groupIncidentMessagesByUser(messages) {
  const map = new Map();
  messages.forEach((msg) => {
    const key = String(msg.user_id);
    if (!map.has(key)) {
      map.set(key, {
        user_id: msg.user_id,
        ...
        handling_status: msg.handling_status || "not_handled",
        messages: []
      });
    }
    const group = map.get(key);
    group.messages.push(msg);
    if (msg.handling_status) group.handling_status = msg.handling_status;
    if (new Date(msg.created_at) > new Date(group.last_at || 0)) group.last_at = msg.created_at;
  });
  return Array.from(map.values()).sort((a, b) => new Date(b.last_at) - new Date(a.last_at));
}

function renderAdminIncidentUsers() {
  const list = $("#adminIncidentUsers");
  const messagesWrap = $("#adminIncidentMessages");
  if (!list) return;
  const groups = groupIncidentMessagesByUser(adminIncidentMessagesCache);
  if (!groups.length) {
    list.innerHTML = '<div class="empty-state">لا توجد رسائل حوادث بعد.</div>';
    if (messagesWrap) messagesWrap.innerHTML = '<div class="empty-state">اختر محادثة لعرض الرسائل.</div>';
    return;
  }
  if (!activeIncidentUserId) activeIncidentUserId = groups[0].user_id;
  list.innerHTML = groups.map((group) => `
    <div class="incident-thread-item ${String(group.user_id) === String(activeIncidentUserId) ? "active" : ""}">
      <button type="button" class="incident-thread-btn" data-user-id="${escapeHtml(group.user_id)}">
        <strong>${escapeHtml(group.user_name)}</strong>
        <span>${escapeHtml(group.user_email)}</span>
        <small>${escapeHtml(formatChatTime(group.last_at))}</small>
      </button>
      <div class="incident-status-row">
        <span class="incident-status-badge ${incidentHandlingStatusClass(group.handling_status)}">
          ${incidentHandlingStatusText(group.handling_status)}
        </span>
        <select class="incident-status-select" data-user-id="${escapeHtml(group.user_id)}">
          <option value="not_handled" ${group.handling_status === "not_handled" ? "selected" : ""}>لم يتم التعامل</option>
          <option value="communicating" ${group.handling_status === "communicating" ? "selected" : ""}>جارٍ التواصل</option>
          <option value="handled" ${group.handling_status === "handled" ? "selected" : ""}>تم التعامل</option>
        </select>
      </div>
    </div>
  `).join("");
  renderAdminIncidentThread();
}

function renderAdminIncidentThread() {
  const group = groupIncidentMessagesByUser(adminIncidentMessagesCache).find((x) => String(x.user_id) === String(activeIncidentUserId));
  const title = $("#adminIncidentThreadTitle");
  const replyForm = $("#adminIncidentReplyForm");
  if (!group) {
    renderIncidentChatMessages([], "#adminIncidentMessages");
    if (title) title.textContent = "رسائل الحوادث";
    if (replyForm) replyForm.classList.add("hidden");
    return;
  }
  if (title) title.textContent = `محادثة: ${group.user_name} - ${group.user_email}`;
  if (replyForm) replyForm.classList.remove("hidden");
  renderIncidentChatMessages(group.messages, "#adminIncidentMessages");
}

async function loadAdminIncidentMessages() {
  const items = await apiFetch("/admin/incident-chat/messages");
  adminIncidentMessagesCache = Array.isArray(items) ? items : [];
  renderAdminIncidentUsers();
}

function initAdminIncidentChat() {
  $("#adminIncidentUsers")?.addEventListener("click", (e) => {
    const btn = e.target.closest(".incident-thread-btn");
    if (!btn) return;
    activeIncidentUserId = btn.dataset.userId;
    renderAdminIncidentUsers();
  });

  $("#adminIncidentUsers")?.addEventListener("change", async (e) => {
    const select = e.target.closest(".incident-status-select");
    if (!select) return;
    try {
      await apiFetch(`/admin/incident-chat/${select.dataset.userId}/status`, {
        method: "PATCH",
        body: { handling_status: select.value }
      });
      await loadAdminIncidentMessages();
      showToast("تم تحديث الحالة", "تم حفظ حالة رسالة الحادث");
    } catch (err) {
      showToast("فشل تحديث الحالة", err.message || "حاول مرة أخرى", "error");
    }
  });

  $("#adminIncidentReplyForm")?.addEventListener("submit", async (e) => {
    e.preventDefault();
    if (!activeIncidentUserId) return;
    const input = $("#adminIncidentReplyText");
    const message = input?.value.trim() || "";
    if (!message) {
      showToast("الرد فارغ", "اكتب الرد قبل الإرسال", "error");
      return;
    }
    try {
      await apiFetch(`/admin/incident-chat/${activeIncidentUserId}/reply`, {
        method: "POST",
        body: { message }
      });
      if (input) input.value = "";
      await loadAdminIncidentMessages();
      showToast("تم الرد", "تم إرسال الرد للمستخدم");
    } catch (err) {
      showToast("فشل الرد", err.message || "تعذر إرسال الرد", "error");
    }
  });

  $("#adminIncidentRefreshBtn")?.addEventListener("click", () => {
    loadAdminIncidentMessages().catch(() => showToast("تعذر التحديث", "حاول مرة أخرى", "error"));
  });

  loadAdminIncidentMessages().catch((err) => {
    const list = $("#adminIncidentUsers");
    const messagesWrap = $("#adminIncidentMessages");
    if (list) list.innerHTML = `<div class="empty-state">تعذر تحميل رسائل الحوادث: ${escapeHtml(err.message || "خطأ غير معروف")}</div>`;
    if (messagesWrap) messagesWrap.innerHTML = '<div class="empty-state">تأكد من تشغيل السيرفر وتطبيق ملف SQL الخاص برسائل الحوادث.</div>';
  });
}
...
function initAdminTabs() {
  ...
  const panels = $$('[data-admin-panel-content]');
  const placeholder = $("#adminPanelPlaceholder");

  function showPanel(name) {
    panels.forEach((panel) => {
      const isActive = panel.dataset.adminPanelContent === name;
      panel.classList.toggle("hidden", !isActive);
    });

    buttons.forEach((btn) => {
      btn.classList.toggle("active", btn.dataset.adminPanel === name);
    });
  ...
  buttons.forEach((btn) => {
    btn.addEventListener("click", () => showPanel(btn.dataset.adminPanel));
  });
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (page === "accidents") initAccidentsPage();
  if (page === "incident-chat") initIncidentChatPage();
